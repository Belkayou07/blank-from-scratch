...
head_size = 64
mlp_hidden = 1365
assert d_model == number_of_heads * head_size
assert head_size % 2 == 0
tokens_per_update = batch_size * context_length
TARGET_TOKEN_PREDICTIONS = 285736960
training_updates = math.ceil(TARGET_TOKEN_PREDICTIONS / tokens_per_update)
actual_token_predictions = training_updates * tokens_per_update
maximum_learning_rate = 0.00025
minimum_learning_rate = 2.5e-05
warmup_updates = 1300
weight_decay = 0.1
gradient_clip = 1.0
...
RESUME = True
BLANK0_BEST_VALIDATION = 3.0686864535013836
...
print('Training updates:', f'{training_updates:,}')
print('Target token predictions:', f'{TARGET_TOKEN_PREDICTIONS:,}')
print('Actual token predictions:', f'{actual_token_predictions:,}')
print('Dataset equivalents:', round(actual_token_predictions / train_tokens, 4))
...
cpu_array = np.asarray(binary_tokens, dtype=np.int32)
...
print('Dataset GPU memory:', round(dataset_memory_gb, 2), 'GB')
...
try:
    optimizer = torch.optim.AdamW(model.parameters(), lr=maximum_learning_rate, betas=(0.9, 0.95), weight_decay=weight_decay, fused=True)
    OPTIMIZER_MODE = 'fused'
except Exception as error:
    print('Fused AdamW unavailable.')
    print('Falling back to foreach AdamW.')
    print('Reason:', error)
    optimizer = torch.optim.AdamW(model.parameters(), lr=maximum_learning_rate, betas=(0.9, 0.95), weight_decay=weight_decay, foreach=True)
...
def get_learning_rate(update_index):
    if update_index < warmup_updates:
        return maximum_learning_rate * (update_index + 1) / warmup_updates
    progress = (update_index - warmup_updates) / (training_updates - warmup_updates)
...
def save_checkpoint(filename, completed_updates, best_validation_loss):
    checkpoint = {'model_name': 'Blank-1 v0.1', 'completed_updates': completed_updates, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'best_validation_loss': best_validation_loss, 'torch_rng_state': torch.get_rng_state(), 'cuda_rng_state': torch.cuda.get_rng_state_all(), 'config': {'vocab_size': vocab_size, 'context_length': context_length, 'batch_size': batch_size, 'd_model': d_model, 'layers': number_of_layers, 'heads': number_of_heads, 'head_size': head_size, 'mlp_hidden': mlp_hidden, 'parameters': parameter_count, 'precision': 'BF16 autocast', 'attention': 'SDPA', 'optimizer': OPTIMIZER_MODE, 'training_updates': training_updates, 'token_predictions': actual_token_predictions}}
...
if RESUME and os.path.exists(LATEST_CHECKPOINT):
    ...
    print('Remaining updates:', f'{training_updates - completed_updates:,}')
if completed_updates == 0:
    print('\nINITIAL EVALUATION')
    initial_metrics = evaluate(evaluation_batches)
    initial_train_loss = initial_metrics['train']['loss']
    initial_validation_loss = initial_metrics['validation']['loss']
    initial_perplexity = initial_metrics['validation']['perplexity']
    best_validation_loss = initial_validation_loss
    print(f'updates={0:5d} | train={initial_train_loss:.4f} | val={initial_validation_loss:.4f} | ppl={initial_perplexity:.2f}')
...
while completed_updates < training_updates:
    ...
    if not torch.isfinite(loss):
        raise RuntimeError(f'Non-finite loss at update {completed_updates + 1}')
    loss.backward()
    gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), gradient_clip)
    if not torch.isfinite(gradient_norm):
        raise RuntimeError(f'Non-finite gradient at update {completed_updates + 1}')
    optimizer.step()
    completed_updates += 1
    updates_since_report += 1
    if completed_updates % evaluation_interval == 0:
        torch.cuda.synchronize()
        now = time.perf_counter()
        training_segment_time = now - report_start
        processed_tokens = updates_since_report * tokens_per_update
        training_throughput = processed_tokens / training_segment_time
        metrics = evaluate(evaluation_batches)
        train_loss = metrics['train']['loss']
        validation_loss = metrics['validation']['loss']
        validation_perplexity = metrics['validation']['perplexity']
        dataset_equivalent = completed_updates * tokens_per_update / train_tokens
        reserved_vram = torch.cuda.memory_reserved(0) / 1024 ** 3
        print(f'updates={completed_updates:5d} | train={train_loss:.4f} | val={validation_loss:.4f} | ppl={validation_perplexity:.2f} | lr={learning_rate:.2e} | grad={gradient_norm.item():.3f} | tok/s={training_throughput:,.0f} | data={dataset_equivalent:.3f}x | VRAM={reserved_vram:.2f} GB')
...
final_metrics = evaluate(final_evaluation_batches)
...
if final_validation_loss < best_validation_loss:
    ...
    save_checkpoint(BEST_CHECKPOINT, completed_updates, best_validation_loss)
    print('Final model became the new best checkpoint.')
print('Best validation loss:', round(best_validation_loss, 4))
difference_vs_blank0 = best_validation_loss - BLANK0_BEST_VALIDATION
print('\nBLANK-0 COMPARISON')
print('Blank-0 best validation:', round(BLANK0_BEST_VALIDATION, 4))
print('Blank-1 best validation:', round(best_validation_loss, 4))
print('Difference:', f'{difference_vs_blank0:+.4f}')
save_checkpoint(FINAL_CHECKPOINT, completed_updates, best_validation_loss)
save_checkpoint(LATEST_CHECKPOINT, completed_updates, best_validation_loss)
print('\nFinal checkpoint:', FINAL_CHECKPOINT)
print('Best checkpoint:', BEST_CHECKPOINT)
print('Latest checkpoint:', LATEST_CHECKPOINT)
session_seconds = training_end - session_start
total_training_predictions = completed_updates * tokens_per_update
average_throughput = total_training_predictions / session_seconds
print('\nPERFORMANCE')
print('Training session time:', round(session_seconds, 2), 'seconds')
print('Training session time:', round(session_seconds / 60, 2), 'minutes')
print('Average overall throughput:', f'{average_throughput:,.0f}', 'tokens/sec')
print('Peak allocated VRAM:', round(torch.cuda.max_memory_allocated(0) / 1024 ** 3, 2), 'GB')
print('Peak reserved VRAM:', round(torch.cuda.max_memory_reserved(0) / 1024 ** 3, 2), 'GB')
...
print('Best checkpoint update:', best_checkpoint['completed_updates'])
print('Best checkpoint validation loss:', best_checkpoint['best_validation_loss'])

@torch.inference_mode()
def generate(prompt, number_of_tokens=250, temperature=0.8, top_k=50):
    model.eval()
