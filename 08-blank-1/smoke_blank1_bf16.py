...
print('Smoke-test updates:', training_updates)
print('Tokens/update:', f'{tokens_per_update:,}')
print('Total token predictions:', f'{training_updates * tokens_per_update:,}')
...
print('Blank-0 parameters: 15,735,168')
print('Blank-1 / Blank-0:', round(parameter_count / 15735168, 3), 'x')
...
print('Expected random CE ≈ ln(vocab):', round(expected_random_loss, 4))
print('Train loss:', round(initial_metrics['train'], 4))
print('Validation loss:', round(initial_metrics['validation'], 4))
if not math.isfinite(initial_metrics['validation']):
    raise RuntimeError('Initial validation loss is not finite.')
print('\nTRAINING SMOKE TEST\n')
model.train()
...
for update in range(training_updates):
    learning_rate = get_learning_rate(update)
    ...
    if not torch.isfinite(loss):
        raise RuntimeError(f'Non-finite loss at update {update + 1}: {loss.item()}')
    loss.backward()
    gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), gradient_clip)
    if not torch.isfinite(gradient_norm):
        raise RuntimeError(f'Non-finite gradient norm at update {update + 1}.')
    optimizer.step()
    segment_updates += 1
    ...
    if completed_updates % evaluation_interval == 0:
        ...
        segment_tokens = segment_updates * tokens_per_update
        throughput = segment_tokens / elapsed
        metrics = evaluate()
        peak_allocated = torch.cuda.max_memory_allocated(0) / 1024 ** 3
        peak_reserved = torch.cuda.max_memory_reserved(0) / 1024 ** 3
        print(f"updates={completed_updates:4d} | train={metrics['train']:.4f} | val={metrics['validation']:.4f} | batch_loss={loss.item():.4f} | grad={gradient_norm.item():.3f} | lr={learning_rate:.2e} | tok/s={throughput:,.0f} | VRAM={peak_reserved:.2f} GB")
        if not math.isfinite(metrics['validation']):
...
total_predictions = training_updates * tokens_per_update
average_training_throughput = total_predictions / session_elapsed
peak_allocated = torch.cuda.max_memory_allocated(0) / 1024 ** 3
peak_reserved = torch.cuda.max_memory_reserved(0) / 1024 ** 3
print('\n' + '=' * 70)
...
print('Completed updates:', training_updates)
print('Token predictions:', f'{total_predictions:,}')
print('Initial train loss:', round(initial_metrics['train'], 4))
print('Initial validation loss:', round(initial_metrics['validation'], 4))
print('Final train loss:', round(final_metrics['train'], 4))
print('Final validation loss:', round(final_metrics['validation'], 4))
print('Total session time:', round(session_elapsed, 2), 'seconds')
print('Whole-session average:', f'{average_training_throughput:,.0f}', 'tokens/sec')
print('Peak allocated VRAM:', round(peak_allocated, 2), 'GB')
print('Peak reserved VRAM:', round(peak_reserved, 2), 'GB')
loss_improved = final_metrics['validation'] < initial_metrics['validation']
finite_final_loss = math.isfinite(final_metrics['validation'])
print('\nHEALTH CHECK')
print('Finite final loss:', finite_final_loss)
print('Validation loss decreased:', loss_improved)
if finite_final_loss and loss_improved:
    print('\nRESULT: BLANK-1 BF16 TRAINING LOOKS HEALTHY.')
else:
    print('\nRESULT: SOMETHING NEEDS INVESTIGATION.')
print('\nNo checkpoint was saved.')
print('This was only the Blank-1 smoke test.')
print('\nDONE')
